...
const char *spcmd1[] = {TERMINAL, "-n", "spterm", "-f", "SF Mono:size=14","-g", "120x34", NULL };
const char *spcmd2[] = {TERMINAL, "-n", "spcalc", "-f", "SF Mono:size=14", "-g", "50x20", "-e", "bc", "-lq", NULL };
...
static Key keys[] = {
	...
	{ MODKEY,			XK_n,		spawn,		SHCMD(TERMINAL " -e nvim -c WikiIndex") },
	...
	/* /1* Client position is limited to monitor window area *1/ */
	...
	/* /1* Absolute positioning (allows moving windows between monitors) *1/ */
	...
	/* /1* Resize client, client center position is fixed which means that client expands in all directions *1/ */
	...
	/* /1* Client is positioned in a floating grid, movement is relative to client's current position *1/ */
